...
let settings = {};
let tab = 'services';
let services = [], rentals = [], deposits = [], users = [], adminRentals = [], adminDeposits = [], notifications = [], dmxProducts = [], dmxOrders = [], highlandsStock = [];
...
function menu(){
  ...
  const admin = [['admin_services','Dịch vụ admin'],['admin_highlands','Kho Highlands'],['admin_dmx','Quản lý DMX'],['admin_api','API thuê sim'],['admin_history','Lịch sử admin'],['admin_deposit_info','Nạp tiền admin'],['admin_users','Quản lý người dùng'],['admin_web','Quản lý web'],['admin_approve','Duyệt nạp tiền']];
...
async function renderTab(){
  ...
  if(tab==='admin_highlands') return adminHighlands();
...
async function createSepayDeposit(){
  ...
  try{
    ...
    const qrImg = d.qrUrl ? `<img class="qr" src="${esc(d.qrUrl)}" onerror="if('${esc(d.qrUrlAlt||'')}'){this.onerror=null;this.src='${esc(d.qrUrlAlt||'')}';}else{this.style.display='none';}">` : '';
    const qrLink = d.qrUrl ? `<p><a href="${esc(d.qrUrl)}" target="_blank">Mở QR trong tab mới</a></p>` : '';
    $('#sepayBox').innerHTML = `<div class="notice okbox" style="margin-top:12px"><b>Chuyển khoản đúng nội dung:</b><br><code>${esc(d.transferContent)}</code><br><b>Số tiền:</b> ${fmt(d.deposit.amount)}<br><b>Tài khoản:</b> ${esc(d.bank)} - ${esc(d.account)} - ${esc(d.accountName||'')}</div>${qrImg}${qrLink}<p class="muted">Nếu ảnh QR chưa hiện, bấm link mở QR trong tab mới. Sau khi chuyển khoản, chờ vài giây rồi tải lại lịch sử nếu số dư chưa cập nhật.</p>`;
...
async function addService(){ try{ let imageUrl=''; if($('#sImage')?.files[0]) imageUrl=await uploadFile($('#sImage')); await api('/api/admin/services',{method:'POST',body:JSON.stringify({service_type:$('#sType').value,name:$('#sName').value,external_app_id:$('#sAppId').value,network:$('#sNet').value,price:$('#sPrice').value,description:$('#sDesc').value,imageUrl,visible:false})}); await loadPage(); }catch(e){ toast(e.message,false); } }
async function saveService(id){ let imageUrl=$('#img_'+id)?.value||''; if($('#file_'+id)?.files[0]) imageUrl=await uploadFile($('#file_'+id)); await api('/api/admin/services/'+id,{method:'PATCH',body:JSON.stringify({provider:$('#prov_'+id)?.value||'legacy',service_type:$('#type_'+id)?.value||'sim',name:$('#n_'+id).value,external_app_id:$('#app_'+id).value,network:$('#net_'+id).value,price:$('#p_'+id).value,api_cost:$('#cost_'+id).value,visible:$('#v_'+id).checked,description:$('#d_'+id).value,imageUrl})}); toast('Đã lưu dịch vụ'); services=await api('/api/services'); renderServiceTable(); }

async function adminHighlands(){
  services = await api('/api/services');
  const hlServices = services.filter(s => s.service_type === 'highlands');
  highlandsStock = await api('/api/admin/highlands-stock');
  let orders=[]; try{ orders=await api('/api/admin/highlands-orders'); }catch(e){ orders=[]; }
  $('.main').insertAdjacentHTML('beforeend', `<div class="card"><h2>Admin - Kho số Highlands</h2><p class="muted">ACC Highlands được tạo tự động. Admin paste số vào kho; khi khách mua, web thuê lại đúng số đó bằng API rồi chờ OTP.</p>${!hlServices.length?'<div class="notice danger">Chưa có dịch vụ Highlands. Hãy restart server/deploy lại để tự tạo ACC Highlands.</div>':''}<div class="row"><div class="field"><label>Dịch vụ Highlands</label><select id="hlService">${hlServices.map(s=>`<option value="${s.id}">${esc(s.name)} - Service ID ${esc(s.external_app_id||'')}</option>`).join('')}</select></div></div><div class="field"><label>Danh sách số, mỗi dòng 1 số</label><textarea id="hlPhones" placeholder="0812119032
0812440795"></textarea></div><button onclick="importHighlandsStock()">Nhập kho Highlands</button><button class="secondary" onclick="loadPage()">Làm mới</button></div><div class="card"><h3>Danh sách kho</h3><div class="full-actions"><button class="small secondary" onclick="toggleAllHighlandsStock(true)">Chọn tất cả</button><button class="small secondary" onclick="toggleAllHighlandsStock(false)">Bỏ chọn</button><button class="small danger" onclick="deleteSelectedHighlandsStock()">Xóa đã chọn</button><button class="small danger" onclick="deleteHighlandsByStatus()">Xóa theo trạng thái</button><button class="small danger" onclick="deleteAllHighlandsStock()">Xóa toàn bộ kho đang chọn</button><select id="hlDeleteStatus"><option value="free">Trống</option><option value="hold">Đang giữ/lỗi</option><option value="sold">Đã bán</option></select></div>${tableHighlandsStock(highlandsStock)}</div><div class="card"><h3>Lịch sử ACC Highlands</h3>${tableHighlandsOrders(orders)}</div>`);
}
function tableHighlandsStock(rows){ if(!rows.length) return '<p class="muted">Chưa có số Highlands nào trong kho.</p>'; return `<div class="tablewrap"><table class="table"><tr><th><input type="checkbox" onchange="toggleAllHighlandsStock(this.checked)"></th><th>Dịch vụ</th><th>Số</th><th>Trạng thái</th><th>User</th><th>Ghi chú</th><th>Ngày nhập</th><th>Lưu</th></tr>${rows.map(x=>`<tr><td><input class="hlChk" type="checkbox" value="${x.id}"></td><td>${esc(x.service_name||'')}</td><td><b>${esc(x.phone)}</b></td><td><select id="hlst_${x.id}"><option value="free" ${x.status==='free'?'selected':''}>Trống</option><option value="hold" ${x.status==='hold'?'selected':''}>Đang giữ / lỗi</option><option value="sold" ${x.status==='sold'?'selected':''}>Đã bán</option></select></td><td>${esc(x.user_id||'')}</td><td><input id="hln_${x.id}" value="${esc(x.note||'')}"></td><td>${date(x.created_at)}</td><td><button class="small" onclick="saveHighlandsStock('${x.id}')">Lưu</button><button class="small danger" onclick="deleteHighlandsStock('${x.id}')">Xóa</button></td></tr>`).join('')}</table></div>`; }
function tableHighlandsOrders(rows){ if(!rows.length) return '<p class="muted">Chưa có giao dịch ACC Highlands.</p>'; return `<div class="tablewrap"><table class="table"><tr><th>User</th><th>Dịch vụ</th><th>Số</th><th>Giá</th><th>Trạng thái</th><th>OTP</th><th>Ngày thuê</th><th>Kết thúc</th><th>Ghi chú</th></tr>${rows.map(r=>`<tr><td>${esc(r.username||'')}</td><td>${esc(r.service_name||'')}</td><td><b>${esc(r.phone_number||'')}</b></td><td>${fmt(r.price||0)}</td><td>${esc(r.status||'')}</td><td>${esc(r.otp_code||'')}</td><td>${date(r.rented_at)}</td><td>${date(r.ended_at)}</td><td>${esc(r.note||'')}</td></tr>`).join('')}</table></div>`; }
async function importHighlandsStock(){ try{ const service_id=$('#hlService').value; const phones=$('#hlPhones').value; const d=await api('/api/admin/highlands-stock/import',{method:'POST',body:JSON.stringify({service_id,phones})}); toast(`Đã nhập ${d.added} số, bỏ qua ${d.skipped} số trùng/sai`); await loadPage(); }catch(e){ toast(e.message,false); } }
async function saveHighlandsStock(id){ try{ await api('/api/admin/highlands-stock/'+id,{method:'PATCH',body:JSON.stringify({status:$('#hlst_'+id).value,note:$('#hln_'+id).value})}); toast('Đã lưu số Highlands'); }catch(e){ toast(e.message,false); } }
async function deleteHighlandsStock(id){ if(!confirm('Xóa số này khỏi kho?')) return; await api('/api/admin/highlands-stock/'+id,{method:'DELETE'}); toast('Đã xóa'); await loadPage(); }
function selectedHighlandsStockIds(){ return $$('.hlChk').filter(x=>x.checked).map(x=>x.value); }
function toggleAllHighlandsStock(checked){ $$('.hlChk').forEach(x=>x.checked=checked); }
async function deleteSelectedHighlandsStock(){ const ids=selectedHighlandsStockIds(); if(!ids.length) return toast('Chưa chọn số nào',false); if(!confirm('Xóa '+ids.length+' số đã chọn?')) return; const d=await api('/api/admin/highlands-stock/bulk-delete',{method:'POST',body:JSON.stringify({ids})}); toast('Đã xóa '+d.deleted+' số'); await loadPage(); }
async function deleteHighlandsByStatus(){ const status=$('#hlDeleteStatus').value; const service_id=$('#hlService')?$('#hlService').value:''; if(!confirm('Xóa tất cả số trạng thái '+status+' của dịch vụ đang chọn?')) return; const d=await api('/api/admin/highlands-stock/bulk-delete',{method:'POST',body:JSON.stringify({service_id,status})}); toast('Đã xóa '+d.deleted+' số'); await loadPage(); }
async function deleteAllHighlandsStock(){ const service_id=$('#hlService')?$('#hlService').value:''; if(!confirm('Xóa toàn bộ kho Highlands của dịch vụ đang chọn?')) return; const d=await api('/api/admin/highlands-stock/bulk-delete',{method:'POST',body:JSON.stringify({service_id,delete_all:true})}); toast('Đã xóa '+d.deleted+' số'); await loadPage(); }
...
async function adminApi(){
  ...
  <div class="field"><label>Thời gian chờ OTP trước khi tự hoàn tiền (phút)</label><input id="otpTimeoutMinutes" type="number" min="1" value="${esc(settings.otpTimeoutMinutes||'5')}"></div><div class="flex"><button onclick="saveApiSettings()">Lưu 2 API key</button><button class="secondary" onclick="testBothApi()">Test cả 2 API</button><button class="secondary" onclick="syncApiApps()">Đồng bộ dịch vụ từ cả 2 API</button></div><div id="apiResult" class="notice" style="white-space:pre-wrap;margin-top:12px"></div></div>`);
}
