...
async function adminUsers(){ users=await api('/api/admin/users?limit=300'); globalStats=await api('/api/stats/global').catch(()=>null); $('.main').insertAdjacentHTML('beforeend', `<div class="card"><h2>Thống kê toàn web từ 06/07</h2><div class="stats"><span class="pill">Tổng nạp: <b>${fmt(globalStats?.deposits_total||0)}</b></span><span class="pill">Tổng sử dụng: <b>${fmt(globalStats?.used_total||0)}</b></span><span class="pill">Tổng chuyển tiền: <b>${fmt(globalStats?.transfer_total||0)}</b></span><span class="pill">User: <b>${globalStats?.users_total||0}</b></span></div></div><div class="card"><h2>Admin - Quản lý người dùng</h2><p class="muted">Chuyển tiền mặc định bị khóa. Admin bật từng tài khoản ở cột Chuyển tiền.</p><div class="field"><label>Tìm kiếm người dùng</label><input id="userSearch" placeholder="Nhập username, quyền hoặc trạng thái" oninput="renderUserTable()"></div><div id="userTable">${tableUsers(users)}</div></div>`); }
...
function tableUsers(rows){ return `<div class="tablewrap"><table class="table"><tr><th>User</th><th>Quyền</th><th>Số dư</th><th>Nạp từ 06/07</th><th>Sử dụng từ 06/07</th><th>Đã chuyển</th><th>Đã nhận</th><th>Chuyển tiền</th><th>Cộng/trừ tiền</th><th>Mật khẩu mới</th><th>Chưa truy cập</th><th>Trạng thái</th><th>Thao tác</th></tr>${rows.map(u=>`<tr><td>${esc(u.username)}</td><td>${esc(u.role)}</td><td><input id="bal_${u.id}" type="number" value="${u.balance}"></td><td>${fmt(u.deposits_total||0)}</td><td>${fmt(u.used_total||0)}</td><td>${fmt(u.transfer_sent||0)}</td><td>${fmt(u.transfer_received||0)}</td><td><select id="transfer_${u.id}"><option value="0" ${u.transfer_enabled?'':'selected'}>Khóa</option><option value="1" ${u.transfer_enabled?'selected':''}>Mở</option></select></td><td><input id="add_${u.id}" type="number" placeholder="+/-"></td><td><input id="pass_${u.id}" placeholder="Bỏ trống nếu không đổi"></td><td>${u.days_inactive||0} ngày</td><td>${esc(u.status)}</td><td><button class="small" onclick="saveUser('${u.id}')">Lưu</button> <button class="small danger" onclick="deleteUser('${u.id}')">Xóa</button></td></tr>`).join('')}</table></div>`; }
async function saveUser(id){ const body={balance:$('#bal_'+id).value, transfer_enabled: $('#transfer_'+id).value === '1'}; if($('#add_'+id).value) body.addBalance=$('#add_'+id).value; if($('#pass_'+id).value) body.password=$('#pass_'+id).value; await api('/api/admin/users/'+id,{method:'PATCH',body:JSON.stringify(body)}); toast('Đã lưu user'); await loadMe(); await loadPage(); }
