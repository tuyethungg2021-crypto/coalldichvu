...
function tableHighlandsStock(rows){ if(!rows.length) return '<p class="muted">Chưa có số Highlands nào trong kho.</p>'; return `<div class="tablewrap"><table class="table"><tr><th>Dịch vụ</th><th>Số</th><th>Trạng thái</th><th>User</th><th>Ghi chú</th><th>Ngày nhập</th><th>Lưu</th></tr>${rows.map(x=>`<tr><td>${esc(x.service_name||'')}</td><td><b>${esc(x.phone)}</b></td><td><select id="hlst_${x.id}"><option value="free" ${x.status==='free'?'selected':''}>Trống</option><option value="sold" ${x.status==='sold'?'selected':''}>Đã bán</option><option value="hold" ${x.status==='hold'?'selected':''}>Giữ lại</option></select></td><td>${esc(x.user_id||'')}</td><td><input id="hln_${x.id}" value="${esc(x.note||'')}"></td><td>${date(x.created_at)}</td><td><button class="small" onclick="saveHighlandsStock('${x.id}')">Lưu</button><button class="small danger" onclick="deleteHighlandsStock('${x.id}')">Xóa</button></td></tr>`).join('')}</table></div>`; }
