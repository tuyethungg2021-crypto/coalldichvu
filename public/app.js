...
async function checkCode(id){ try{ const d=await api('/api/rentals/'+id+'/check-code',{method:'POST',body:JSON.stringify({})}); const msg=d.api?.message || d.api?.Msg || 'Đã kiểm tra code'; toast(msg + (d.rental.otp_code?(': '+d.rental.otp_code):'')); await loadPage(); }catch(e){ toast(e.message,false); } }
...
async function addService(){ try{ let imageUrl=''; if($('#sImage')?.files[0]) imageUrl=await uploadFile($('#sImage')); await api('/api/admin/services',{method:'POST',body:JSON.stringify({name:$('#sName').value,provider:$('#sProvider').value,external_app_id:$('#sAppId').value,network:$('#sNet').value,price:$('#sPrice').value,description:$('#sDesc').value,imageUrl,visible:false})}); await loadPage(); }catch(e){ toast(e.message,false); } }
async function saveService(id){ let imageUrl=$('#img_'+id)?.value||''; if($('#file_'+id)?.files[0]) imageUrl=await uploadFile($('#file_'+id)); await api('/api/admin/services/'+id,{method:'PATCH',body:JSON.stringify({name:$('#n_'+id).value,provider:$('#prov_'+id).value,external_app_id:$('#app_'+id).value,network:$('#net_'+id).value,price:$('#p_'+id).value,api_cost:$('#cost_'+id).value,visible:$('#v_'+id).checked,description:$('#d_'+id).value,imageUrl})}); toast('Đã lưu dịch vụ'); await loadPage(); }
async function adminApi(){
  $('.main').insertAdjacentHTML('beforeend', `<div class="card"><h2>Admin - API thuê sim</h2><p class="muted">Cấu hình đồng thời 2 nguồn API. Khi bấm đồng bộ, hệ thống gọi cả Legacy cũ và CodeSim, dịch vụ mới mặc định ẩn. Khi user thuê, hệ thống tự gọi đúng API theo nguồn của dịch vụ.</p>
  <div class="row2">
    <div class="card mini"><h3>API 1 - Legacy cũ</h3>
      <div class="field"><label>API URL Legacy</label><input id="legacyApiBaseUrl" value="${esc(settings.legacyApiBaseUrl||settings.apiBaseUrl||'https://chaycodeso3.com/api')}"></div>
      <div class="field"><label>API key Legacy</label><input id="legacyApiKey" value="${esc(settings.legacyApiKey||'')}" placeholder="Nhập API key API cũ"></div>
      <button class="secondary" onclick="testApiAccount('legacy')">Test Legacy</button>
      <div class="muted">Key hiện tại: ${settings.legacyApiKeyMasked?esc(settings.legacyApiKeyMasked):'Chưa cài'}</div>
    </div>
    <div class="card mini"><h3>API 2 - CodeSim</h3>
      <div class="field"><label>API URL CodeSim</label><input id="codesimApiBaseUrl" value="${esc(settings.codesimApiBaseUrl||'https://apisim.codesim.net')}"></div>
      <div class="field"><label>API key CodeSim</label><input id="codesimApiKey" value="${esc(settings.codesimApiKey||settings.apiKey||'')}" placeholder="Nhập API key CodeSim"></div>
      <button class="secondary" onclick="testApiAccount('codesim')">Test CodeSim</button>
      <div class="muted">Key hiện tại: ${settings.codesimApiKeyMasked?esc(settings.codesimApiKeyMasked):'Chưa cài'}</div>
    </div>
  </div>
  <div class="field"><label>Thời gian chờ OTP trước khi tự hoàn tiền (phút)</label><input id="otpTimeoutMinutes" type="number" min="1" value="${esc(settings.otpTimeoutMinutes||'20')}"></div>
  <div class="flex"><button onclick="saveApiSettings()">Lưu 2 API key</button><button class="secondary" onclick="testAllApiAccounts()">Test cả 2 API</button><button class="secondary" onclick="syncApiApps()">Đồng bộ dịch vụ từ cả 2 API</button></div>
  <div id="apiResult" class="notice" style="white-space:pre-wrap;margin-top:12px">Dịch vụ sync mới sẽ ẩn. Bạn vào Dịch vụ admin để bật từng dịch vụ muốn bán.</div></div>`);
}
async function saveApiSettings(){
  settings=await api('/api/admin/settings',{method:'PATCH',body:JSON.stringify({
    legacyApiBaseUrl:$('#legacyApiBaseUrl').value,
    legacyApiKey:$('#legacyApiKey').value,
    codesimApiBaseUrl:$('#codesimApiBaseUrl').value,
    codesimApiKey:$('#codesimApiKey').value,
    otpTimeoutMinutes:$('#otpTimeoutMinutes').value
  })});
  toast('Đã lưu 2 API key'); await loadSettings();
}
async function testApiAccount(provider='codesim'){
  try{ const d=await api('/api/admin/sim-api/account?provider='+encodeURIComponent(provider)); $('#apiResult').textContent=JSON.stringify(d,null,2); }
  catch(e){ $('#apiResult').textContent=e.message; toast(e.message,false); }
}
async function testAllApiAccounts(){
  try{ const d=await api('/api/admin/sim-api/accounts'); $('#apiResult').textContent=JSON.stringify(d,null,2); }
  catch(e){ $('#apiResult').textContent=e.message; toast(e.message,false); }
}
