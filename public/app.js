...
let settings = {};
let tab = 'services';
let services = [], rentals = [], deposits = [], users = [], adminRentals = [], adminDeposits = [], notifications = [], dmxProducts = [], dmxOrders = [], transfers = [];
let globalStats = null;
const TRANSFER_MIN_AMOUNT = 20000;
...
function menu(){
  const common = [['services','Dịch vụ'],['dmx','Dịch Vụ DMX'],['history','Lịch sử'],['deposit','Nạp tiền'],['transfer','Chuyển tiền']];
  const admin = [['admin_services','Dịch vụ admin'],['admin_dmx','Quản lý DMX'],['admin_api','API thuê sim'],['admin_history','Lịch sử admin'],['admin_deposit_info','Binance'],['admin_users','Quản lý người dùng'],['admin_web','Quản lý web'],['admin_approve','Duyệt nạp tiền']];
...
async function renderTab(){
  ...
  if(tab==='transfer') return userTransfer();
...
async function userDeposit(){
  ...
  const depositNotice = settings.binanceEnabled === '1' ? '' : '<div class="card"><h2>Nạp tiền</h2><p class="muted">Hiện chỉ hỗ trợ Binance Pay. Admin cần bật Binance Pay trong mục Binance.</p></div>';
  $('.main').insertAdjacentHTML('beforeend', `${binanceCard}${depositNotice}<div class="card"><h2>Lịch sử nạp</h2>${tableDeposits(deposits)}</div>`);
}
...
function tableTransfers(rows){
  if(!rows.length) return '<p class="muted">Chưa có giao dịch chuyển tiền.</p>';
  return `<div class="tablewrap"><table class="table"><tr><th>Thời gian</th><th>Người gửi</th><th>Người nhận</th><th>Số tiền</th><th>Ghi chú</th></tr>${rows.map(t=>`<tr><td>${date(t.created_at)}</td><td>${esc(t.from_username||'')}</td><td>${esc(t.to_username||'')}</td><td>${fmt(t.amount)}</td><td>${esc(t.note||'')}</td></tr>`).join('')}</table></div>`;
}
async function userTransfer(){
  transfers = await api('/api/transfers').catch(()=>[]);
  const locked = !me.transfer_enabled;
  $('.main').insertAdjacentHTML('beforeend', `<div class="card"><h2>Chuyển tiền</h2>${locked?'<div class="notice err">Tài khoản của bạn đang bị khóa chức năng chuyển tiền. Admin cần mở thì mới chuyển được.</div>':'<div class="notice okbox">Tài khoản của bạn đã được mở chức năng chuyển tiền.</div>'}<form onsubmit="sendTransfer(event)"><div class="row"><div class="field"><label>Tài khoản người nhận</label><input id="transferReceiver" placeholder="Nhập username hoặc UID" ${locked?'disabled':''}></div><div class="field"><label>Số tiền chuyển</label><input id="transferAmount" type="number" min="20000" step="1000" placeholder="Tối thiểu 20.000đ" ${locked?'disabled':''}></div></div><div class="field"><label>Ghi chú</label><input id="transferNote" maxlength="200" placeholder="Không bắt buộc" ${locked?'disabled':''}></div><button ${locked?'disabled':''}>Chuyển tiền</button></form></div><div class="card"><h2>Lịch sử chuyển tiền</h2>${tableTransfers(transfers)}</div>`);
}
async function sendTransfer(e){
  e.preventDefault();
  if(!confirm('Xác nhận chuyển tiền?')) return;
  try{
    const d = await api('/api/transfers',{method:'POST',body:JSON.stringify({receiver:$('#transferReceiver').value, amount:$('#transferAmount').value, note:$('#transferNote').value})});
    me = d.user;
...
async function adminUsers(){ users=await api('/api/admin/users?limit=300'); globalStats=await api('/api/stats/global').catch(()=>null); $('.main').insertAdjacentHTML('beforeend', `<div class="card"><h2>Thống kê toàn web từ 06/07/2026 đến hiện tại</h2><div class="stats"><span class="pill">Tổng số dư hiện tại toàn web: <b>${fmt(globalStats?.balance_total||0)}</b></span><span class="pill">Tổng tiền toàn web đã nạp: <b>${fmt(globalStats?.deposits_total||0)}</b></span><span class="pill">Tổng tiền toàn web đã sử dụng: <b>${fmt(globalStats?.used_total||0)}</b></span><span class="pill">Tổng tiền user đã chuyển: <b>${fmt(globalStats?.transfer_total||0)}</b></span><span class="pill">Tổng user: <b>${globalStats?.users_total||0}</b></span><span class="pill">Tiền mất trong acc API Key thuê sim: <b>${fmt(globalStats?.api_key_sim_cost_total||0)}</b></span></div><p class="muted">Số dư là tính tại thời điểm hiện tại; các số nạp/sử dụng/chuyển/API key chỉ tính từ 00:00 ngày 06/07/2026. Mục "Tiền mất trong acc API Key thuê sim" chỉ tính chi phí thực trả cho nhà cung cấp API thuê sim (không tính DMX).</p></div><div class="card"><h2>Admin - Quản lý người dùng</h2><div class="notice err">Mặc định mọi tài khoản đều bị KHÓA chuyển tiền. Admin phải chọn “Mở” rồi bấm Lưu cho từng user thì user đó mới chuyển được. Min chuyển: 20.000đ.</div><div class="field"><label>Tìm kiếm người dùng</label><input id="userSearch" placeholder="Nhập username, quyền hoặc trạng thái" oninput="renderUserTable()"></div><div id="userTable">${tableUsers(users)}</div></div>`); }
...
function tableUsers(rows){ return `<div class="tablewrap"><table class="table usertable"><tr><th>User</th><th>Quyền</th><th>Số dư</th><th>Nạp từ 06/07</th><th>SD từ 06/07</th><th>Đã chuyển</th><th>Đã nhận</th><th>Chuyển tiền</th><th>+/-</th><th>Mật khẩu mới</th><th>Chưa truy cập</th><th>Trạng thái</th><th>Thao tác</th></tr>${rows.map(u=>`<tr><td><b>${esc(u.username)}</b></td><td>${esc(u.role)}</td><td><input class="w-sm" id="bal_${u.id}" type="number" value="${u.balance}"></td><td>${fmt(u.deposits_total||0)}</td><td>${fmt(u.used_total||0)}</td><td>${fmt(u.transfer_sent||0)}</td><td>${fmt(u.transfer_received||0)}</td><td><select class="w-sm" id="transfer_${u.id}"><option value="0" ${u.transfer_enabled?'':'selected'}>🔒 Khóa</option><option value="1" ${u.transfer_enabled?'selected':''}>✅ Mở</option></select></td><td><input class="w-xs" id="add_${u.id}" type="number" placeholder="+/-"></td><td><input class="w-md" id="pass_${u.id}" placeholder="Không đổi"></td><td>${u.days_inactive||0}n</td><td>${esc(u.status)}</td><td><button class="small" onclick="saveUser('${u.id}')">Lưu</button> <button class="small danger" onclick="deleteUser('${u.id}')">Xóa</button></td></tr>`).join('')}</table></div>`; }
async function saveUser(id){ const body={balance:$('#bal_'+id).value, transfer_enabled: $('#transfer_'+id).value === '1'}; if($('#add_'+id).value) body.addBalance=$('#add_'+id).value; if($('#pass_'+id).value) body.password=$('#pass_'+id).value; await api('/api/admin/users/'+id,{method:'PATCH',body:JSON.stringify(body)}); toast(body.transfer_enabled ? 'Đã lưu user và MỞ chuyển tiền cho tài khoản này' : 'Đã lưu user và KHÓA chuyển tiền cho tài khoản này'); await loadMe(); await loadPage(); }
