...
function tableUsers(rows){ return `<div class="tablewrap"><table class="table"><tr><th>User</th><th>Quyền</th><th>Số dư</th><th>Nạp từ 06/07</th><th>Sử dụng từ 06/07</th><th>Đã chuyển</th><th>Đã nhận</th><th>Quyền chuyển tiền</th><th>Cộng/trừ tiền</th><th>Mật khẩu mới</th><th>Chưa truy cập</th><th>Trạng thái</th><th>Thao tác</th></tr>${rows.map(u=>`<tr><td><b>${esc(u.username)}</b></td><td>${esc(u.role)}</td><td><input id="bal_${u.id}" type="number" value="${u.balance}"></td><td>${fmt(u.deposits_total||0)}</td><td>${fmt(u.used_total||0)}</td><td>${fmt(u.transfer_sent||0)}</td><td>${fmt(u.transfer_received||0)}</td><td><select id="transfer_${u.id}"><option value="0" ${u.transfer_enabled?'':'selected'}>🔒 Khóa</option><option value="1" ${u.transfer_enabled?'selected':''}>✅ Mở</option></select><div class="muted">${u.transfer_enabled?'Đang được chuyển':'Đang bị khóa'}</div></td><td><input id="add_${u.id}" type="number" placeholder="+/-"></td><td><input id="pass_${u.id}" placeholder="Bỏ trống nếu không đổi"></td><td>${u.days_inactive||0} ngày</td><td>${esc(u.status)}</td><td><button class="small" onclick="saveUser('${u.id}')">Lưu</button> <button class="small danger" onclick="deleteUser('${u.id}')">Xóa</button></td></tr>`).join('')}</table></div>`; }
