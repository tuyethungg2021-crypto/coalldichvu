...
async function userServices(){
  ...
  $('.main').insertAdjacentHTML('beforeend', `<div class="card"><h2>Dịch vụ thuê sim</h2><p class="muted">Chọn dịch vụ và nhà mạng muốn thuê. Web sẽ gọi API thật để lấy số, sau đó bạn bấm Lấy code.</p><div class="servicegrid">${services.map(s=>`<div class="svc"><h3>${esc(s.name)}</h3><p><span class="badge">App ID: ${esc(s.external_app_id||'chưa gắn')}</span></p><p class="muted">${esc(s.description||'')}</p><div class="field"><label>Nhà mạng</label><select id="carrier_${s.id}">${carrierOptions(s.network)}</select></div><div class="price">${fmt(s.price)}</div><button onclick="rent('${s.id}')">Thuê sim</button></div>`).join('')}</div></div><div class="card"><h2>Sim đang thuê</h2><div id="activeRentals"></div></div>`);
...
async function checkCode(id){ try{ const d=await api('/api/rentals/'+id+'/check-code',{method:'POST',body:JSON.stringify({})}); const msg=d.api?.Msg || 'Đã kiểm tra code'; toast(msg + (d.rental.otp_code?(': '+d.rental.otp_code):'')); await loadPage(); }catch(e){ toast(e.message,false); } }
async function cancelRental(id){ if(!confirm('Hủy lượt thuê này?')) return; try{ const d=await api('/api/rentals/'+id+'/cancel',{method:'POST',body:JSON.stringify({})}); toast(d.api?.Msg || 'Đã gửi yêu cầu hủy'); await loadPage(); }catch(e){ toast(e.message,false); } }
function tableRentals(rows){ if(!rows.length) return '<p class="muted">Chưa có dữ liệu.</p>'; return `<div class="tablewrap"><table class="table"><tr><th>Dịch vụ</th><th>Nhà mạng</th><th>Số sim</th><th>Giá</th><th>Trạng thái</th><th>OTP/SMS</th><th>Thời gian</th><th>Thao tác</th></tr>${rows.map(r=>`<tr><td>${esc(r.service_name)}</td><td>${esc(r.network)}</td><td><b>${esc(r.phone_number)}</b><br><small>${esc(r.external_id?'API ID '+r.external_id:'')}</small></td><td>${fmt(r.price)}</td><td><span class="badge">${esc(r.status)}</span><br><small>${esc(r.note||'')}</small></td><td><b>${esc(r.otp_code||'Chưa có')}</b><br><small>${esc(r.sms||'')}</small></td><td>${date(r.rented_at)}</td><td>${r.external_id?`<button class="small ok" onclick="checkCode('${r.id}')">Lấy code</button> <button class="small danger" onclick="cancelRental('${r.id}')">Hủy</button>`:''} ${r.service_id?`<button class="small" onclick="rent('${r.service_id}')">Thuê lại</button>`:''}</td></tr>`).join('')}</table></div>`; }
...
async function addService(){ try{ await api('/api/admin/services',{method:'POST',body:JSON.stringify({name:$('#sName').value,external_app_id:$('#sAppId').value,network:$('#sNet').value,price:$('#sPrice').value,description:$('#sDesc').value,visible:true})}); await loadPage(); }catch(e){ toast(e.message,false); } }
