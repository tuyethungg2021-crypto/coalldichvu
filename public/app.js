...
async function adminHighlands(){
  ...
  $('.main').insertAdjacentHTML('beforeend', `<div class="card"><h2>Admin - Kho số Highlands</h2><p class="muted">Chọn dịch vụ Highlands, paste danh sách số điện thoại vào đây. Khi khách mua, web sẽ lấy số trạng thái Trống theo thứ tự trong kho, gọi API thuê lại đúng số đó rồi chờ OTP.</p><div class="row"><div class="field"><label>Dịch vụ Highlands</label><select id="hlService">${hlServices.map(s=>`<option value="${s.id}">${esc(s.name)} - ${esc(s.external_app_id||'')}</option>`).join('')}</select></div></div><div class="field"><label>Danh sách số, mỗi dòng 1 số</label><textarea id="hlPhones" placeholder="0901234567\n0912345678"></textarea></div><button onclick="importHighlandsStock()">Nhập kho Highlands</button><button class="secondary" onclick="loadPage()">Làm mới</button></div><div class="card"><h3>Danh sách kho</h3>${tableHighlandsStock(highlandsStock)}</div>`);
}
function tableHighlandsStock(rows){ if(!rows.length) return '<p class="muted">Chưa có số Highlands nào trong kho.</p>'; return `<div class="tablewrap"><table class="table"><tr><th>Dịch vụ</th><th>Số</th><th>Trạng thái</th><th>User</th><th>Ghi chú</th><th>Ngày nhập</th><th>Lưu</th></tr>${rows.map(x=>`<tr><td>${esc(x.service_name||'')}</td><td><b>${esc(x.phone)}</b></td><td><select id="hlst_${x.id}"><option value="free" ${x.status==='free'?'selected':''}>Trống</option><option value="hold" ${x.status==='hold'?'selected':''}>Đang giữ / lỗi</option><option value="sold" ${x.status==='sold'?'selected':''}>Đã bán</option></select></td><td>${esc(x.user_id||'')}</td><td><input id="hln_${x.id}" value="${esc(x.note||'')}"></td><td>${date(x.created_at)}</td><td><button class="small" onclick="saveHighlandsStock('${x.id}')">Lưu</button><button class="small danger" onclick="deleteHighlandsStock('${x.id}')">Xóa</button></td></tr>`).join('')}</table></div>`; }
