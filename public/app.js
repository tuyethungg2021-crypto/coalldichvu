...
async function addService(){ try{ await api('/api/admin/services',{method:'POST',body:JSON.stringify({name:$('#sName').value,external_app_id:$('#sAppId').value,network:$('#sNet').value,price:$('#sPrice').value,description:$('#sDesc').value,visible:false})}); await loadPage(); }catch(e){ toast(e.message,false); } }
